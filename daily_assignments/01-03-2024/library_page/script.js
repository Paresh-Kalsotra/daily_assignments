const url = " http://localhost:8000/library/book/";
...
async function updateTable() {
  ...
  for (let book of booklist) {
    ...
    if (book.isBorrowed === false) {
      td.innerHTML = `<button class="statusbtn" onclick='statusUpdate("${book._id}", ${book.isBorrowed})'>Borrow</button>`;
    } else {
      td.innerHTML = `<button class ="statusbtn" onclick='statusUpdate("${book._id}" , ${book.isBorrowed})'>Return</button>`;
    }
    row.appendChild(td);
  }
}
